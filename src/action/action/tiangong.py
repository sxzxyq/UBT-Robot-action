...
class TianGongRobotController(Node):
    def __init__(self):
        ...
        self.target_similarity_threshold = 0.5  # 目标相似性阈值（米）
        ...
        # 语音相关
        self.voice1_path = "/home/nvidia/data/speech/chenggong.mp3"    # 语音1路径
        self.voice2_path = "/home/nvidia/data/speech/didianliang.mp3"    # 语音2路径
        self.voice3_path = "/home/nvidia/data/speech/anjianyin.mp3"    # 语音3路径
    ...
    def is_same_target(self, pos1, pos2):
        """判断两个位置是否为同一目标"""
        return self.calculate_distance(pos1, pos2) < self.target_similarity_threshold
    ...
    def motion_and_voice(self):
        ...
        if not self.stable_target:
            ...
            # 检查是否为同一目标
            if self.is_same_target(self.stable_target, [self.stop_distance, 0.0, 0.0]):
                ...
                if not self.stable_target:
                    ...
                    if self.is_same_target(self.stable_target, [self.stop_distance, 0.0, 0.0]):
                        ...
                        self.get_logger().info("已佩戴安全帽，但有其他人未佩戴安全帽")
                        self.get_logger().info("执行挥手动作")
                        self.call_motion_number(1)  # 挥手
                        time.sleep(3)
                        self.get_logger().info("播放语音2")
...
def main(args=None):
    ...
    try:
        # 初始等待目标
        controller.wait_for_target(30.0)
