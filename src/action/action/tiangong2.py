...
class TianGongRobotController(Node):
    def __init__(self):
        ...
        self.verification_client = self.create_client(
            TriggerVerification, 
    ...
    def motion_and_voice(self):
        ...
        # response = self.verification_client.call()
        response = self.send_verification_request()
         # 【新增关键修改】检查 response 是否为 None
        if response is None:
            self.get_logger().error("❌ 严重错误：第一次服务调用失败（超时或未连接），任务中止！")
            # 可以在这里选择 return 退出，或者播放一个错误提示语音
            return 
        # 检查目标是否存在
